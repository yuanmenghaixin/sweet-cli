...
var DOCS = {
    ...
    generateDocsByGit(devs, config, gitbranch = 'master') {
        const docsPath = devs.mdPath,
            targetPath = devs.targetPath,
        ...
        const git = spawnSync('git', ['clone', docsPath, downloadPath, '-b', gitbranch]);
        if (git.status === 0) {
            console_log(`文档下载完成`, 'success');
            this.generateDocsByLocal({ mdPath: downloadPath, targetPath: devs.targetPath, oldtargetPath:oldtargetPath }, config);
        } else {
            console_log('git下载出错', 'error');
        }
        ...
        var targetDevs = devs.targetPath !== './' ? devs.targetPath.split(',') : [],
            watchGenerate = targetName => {
                for (var i = 0; i < data.options.length; i++) {
                    ...
                    // git远程端
                    if (gitclone) this.generateDocsByGit({ mdPath, targetPath, oldtargetPath }, config, gitbranch);
